...
# With the file dialog box from:
#   https://web.archive.org/web/20161228115250/https://tkinter.unpythonic.net/wiki/tkFileDialog
class ScrollFrame(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent)  # create a frame (self)

        # Get the screen height and make sure the height is no larger than this
        # (with a bit bit extra for the task bar and title bar)
        windowheight = 500
        screenheight = root.winfo_screenheight() - 125
        if screenheight < windowheight:
            windowheight = screenheight

        self.canvas = tk.Canvas(self, borderwidth=0, background="#ffffff", height=windowheight,
                                width=450)  # place canvas on self
        self.viewPort = tk.Frame(self.canvas,
                                 background="#ffffff")  # place a frame on the canvas, this frame will hold the child
        # widgets
        ...
        self.onFrameConfigure(
            None)  # perform an initial stretch on render, otherwise the scroll region has a tiny border until the
        # first resize
    ...
    def onCanvasConfigure(self, event):
        ...
        self.canvas.itemconfig(self.canvas_window,
                               width=canvas_width)  # whenever the size of the canvas changes alter the window region
        # respectively.
...
class Main(tk.Frame):
    def __init__(self, root):
        ...
        # reportbox: for a summary report of what was processed
        global reportbox
        reportbox = tk.Text(self.scrollFrame.viewPort, height=5, width=50)
        reportbox.insert("end",
                         "Reads AB1 files in the selected folder and returns: Well, Q20 length and full length.\nQ20 "
                         "length is the number of bases with quality at least 20.")
